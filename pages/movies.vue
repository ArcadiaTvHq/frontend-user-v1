...
const handleWatchlistUpdate = (updateData) => {
  // Update the moviesContent array to reflect the new watchlist status
  const contentIndex = moviesContent.value.findIndex(
    (movie) => movie.id === updateData.contentId
  );
  if (contentIndex !== -1) {
    moviesContent.value[contentIndex].in_watch_list = updateData.newStatus;
  }
};
