  ...
  <section class="text-white px-30 flex flex-col">
    ...
    <div class="flex border-b border-1 border-gold gap-3">
      <div v-for="(btn, index) in btns" >
        <button :id="index"  @click="(e)=>handleclick(e)" class="h-15 text-gold  border-b-4 border-white" v-if="btn.isActive.value == true">
        <p>{{ btn.name }}</p>
      </button>
      <button :id="index"  @click="(e)=>handleclick(e)" class="h-15" v-if="btn.isActive.value == false">
        <p>{{ btn.name }}</p>
      </button>
      </div>
...
const all = ref(false)
const movies = ref(true)
const series = ref(false)
const recently = ref(false)
const btns = [{name: 'All', isActive: all}, {name: 'Movies', isActive: movies}, {name: 'Series', isActive: series}, {name: 'Recently Added', isActive: recently}];
...
const style = "h-15 text-gold border-b border-4 border-white "
</script>